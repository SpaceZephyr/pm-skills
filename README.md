...
**1. 新增 `pm-master` 全局总控。** 不知道该用哪个 Skill 时，直接描述你的问题，它负责分诊：判断类问题转顾问团，产出类问题路由到对应执行工具，跨多步的任务编排成工作流链路（例如：需求判断 → PRD → 模拟评审 → 埋点方案），每一步的产出自动变成下一步的输入。

**2. 全部 Skill 打通了上下游。** 每个执行 Skill 现在都声明了自己的上游输入来自谁、产出可以交给谁，链路模式下用「交接摘要」接力，不用每一步重新解释上下文。命名也全部统一成 `pm-*`，互相引用不再失灵。

**3. 三个高频 Skill 深度升级：**

- `pm-prd-writer` 新增「阶段零需求体检」——动笔前先检查需求来源、用户价值、成功定义三个信号，可疑需求建议先过顾问团，避免"更高效地做错事"；待确认项现在必须附默认建议值，评审会上可以直接拍板
- `pm-review-board` 新增评审严格度三档（快速过筛 / 标准 / 上线前终审）和意见质量红线——每条意见必须指到具体章节、说清后果、给出可执行动作，泛泛而谈的意见直接删掉；重大评审支持多子代理并行，角色视角不串味
- `pm-prioritization-engine` 新增敏感性分析（Top 3 排名对哪个参数最敏感、变动多少会翻转）和高分歧裁决——模型打分冲突的需求用俞军的用户价值公式做第三方复核，分歧不再靠平均分掩盖
